...
// IdleBaselineRAPLFromSnap computes idle baselines (P5/min/N) per socket/domain
// from an already-collected snapshot of RAPL samples. It does no collection or
// timing. The snapshot must contain energy counters in mJ per socket/domain.
//
...
func IdleBaselineRAPLFromSnap(
	...
	// Defensive: ensure monotonic ordering by sample mono time.
	// Assumes SampleMeta.Mono is a monotonic timestamp in nanoseconds (uint64).
	s := make([]ring.RaplSample, len(snap))
	copy(s, snap)
	sort.Slice(s, func(i, j int) bool { return s[i].SampleMeta.Mono < s[j].SampleMeta.Mono })

	// Per-socket accumulators of power samples (W).
	...
	for i := 0; i < len(s); i++ {
		...
		if last.ok {
			...
			if cur.SampleMeta.Mono <= last.monoNS {
				// Non-increasing or zero Δt — skip
				goto after
			}
	...
	after:
		...
		// Keep reference to the per-socket counters for next delta step.
		last.sockets = cur.Sockets
	}
	...
	for sock, domMap := range acc {
		...
		for dom, vals := range domMap {
			if len(vals) < 3 {
				continue // too few points for robust stats
			}
			p5, minv, n := DeSpikeP5(vals)
			if n == 0 {
				continue
