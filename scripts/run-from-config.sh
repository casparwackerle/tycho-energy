...
#   METRIC_PATH \
#   BIND_ADDRESS \
#   TYCHO_COLLECTOR_ENABLE_BPF \
#   TYCHO_COLLECTOR_ENABLE_RAPL \
#   TYCHO_COLLECTOR_ENABLE_GPU \
#   TYCHO_COLLECTOR_ENABLE_REDFISH \
#   TYCHO_BPF_POLL_MS \
#   TYCHO_RAPL_POLL_MS \
#   TYCHO_GPU_POLL_MS \
#   TYCHO_REDFISH_POLL_MS \
#   TYCHO_BPF_DELAY_MS \
#   TYCHO_RAPL_DELAY_MS \
#   TYCHO_GPU_DELAY_MS \
#   TYCHO_REDFISH_DELAY_MS \
#   TYCHO_TIMEBASE_QUANTUM_MS \
#   TYCHO_REDFISH_EXPECTED_CHANGE_MS \
#   TYCHO_ANALYSIS_TRIGGER \
#   TYCHO_ANALYSIS_EVERY_SEC \
#   TYCHO_ANALYSIS_DETECT_LONGEST_DELAY
# do
#   safe_export "$k"
# done
mapfile -t EXTRA_KEYS < <(yq -r '.extraEnvVars | keys[]?' "$CONFIG_YAML" 2>/dev/null || true)
for k in "${EXTRA_KEYS[@]}"; do
  safe_export "$k"
done

# (Optional) also support legacy top-level keys not in extraEnvVars
for k in KEPLER_LOG_LEVEL METRIC_PATH BIND_ADDRESS CPU_ARCH_OVERRIDE; do
  safe_export "$k"
done
